...
StringList targetValues = new StringList();
int frame = 0; 
...
void setup(){
 ...
 matrixPos = new PVector(2000, 2000); 
...
void draw(){ // *********************************************** DRAW
  ...
  for(String s: popString(twoAddresses, 9)){// change the 6 to whatever you want for better quality (this does not affect the dimension calculation)
    square P0 = new square(new PVector(width/2, height/2), width, "");
 ...
 saveFrame("data/"+"####--"+DIM.x+".png");  
 
 //targetValues.append(str(frame)+","+str(DIM.x)); 
 
 //saveStrings("targets.txt", targetValues); 
