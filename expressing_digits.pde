...
FloatList dimensions = new FloatList();
PVector matrixPos;
...
void draw(){ // *********************************************** DRAW
  ...
  */
  ...
  for(Vertex V: matrix) V.selected = floor(random(0, 2)) == 0; 
  
  PVector DIM = dim(twoAddresses, nAddresses); 
  dimensions.append(DIM.y);
  
  for(int i = 0; i < dimensions.size(); i ++){
    float dy = map(i, 0, dimensions.size(), 0, 2);
    float ddy = map(i, 0, dimensions.size(), width/2 - 200, width/2 + 200);
    int sum = 0; 
    for(int j = 0; j < dimensions.size(); j++){
      if(approx(dimensions.get(j), dy))  sum+=2; 
    }
    
    ellipse(ddy, height/2 - sum, 10, 10); 
  }
